...
//               className={`flex items-center gap-2 px-4 py-2 rounded-lg text-xs sm:text-sm font-bold transition-all cursor-pointer whitespace-nowrap ${activeTab === 'dashboard'
...
//               className={`flex items-center gap-2 px-4 py-2 rounded-lg text-xs sm:text-sm font-bold transition-all cursor-pointer whitespace-nowrap ${activeTab === 'products'
//                   ? 'bg-indigo-600 text-white shadow-sm'
//                   : 'text-slate-400 hover:text-white hover:bg-slate-800/60'
//                 }`}
//             >
...
//               className={`flex items-center gap-2 px-4 py-2 rounded-lg text-xs sm:text-sm font-bold transition-all cursor-pointer whitespace-nowrap ${activeTab === 'users'
//                   ? 'bg-indigo-600 text-white shadow-sm'
//                   : 'text-slate-400 hover:text-white hover:bg-slate-800/60'
//                 }`}
//             >
...
//               className={`flex items-center gap-2 px-4 py-2 rounded-lg text-xs sm:text-sm font-bold transition-all cursor-pointer whitespace-nowrap ${activeTab === 'others'
//                   ? 'bg-indigo-600 text-white shadow-sm'
//                   : 'text-slate-400 hover:text-white hover:bg-slate-800/60'
//                 }`}
//             >
...
//                         <td className="p-3">
//                           <span
//                             className={`px-2.5 py-1 rounded-full text-xs font-bold inline-block ${ord.status === 'Delivered'
...
//                         <td className="p-4">
//                           <span
//                             className={`px-2.5 py-1 rounded-full text-xs font-bold ${prod.stock > 10
//                                 ? 'bg-emerald-50 text-emerald-700 dark:bg-emerald-950/60 dark:text-emerald-300'
//                                 : prod.stock > 0
//                                   ? 'bg-amber-50 text-amber-700 dark:bg-amber-950/60 dark:text-amber-300'
...
//                         <td className="p-4">
//                           <span
//                             className={`px-2.5 py-1 rounded-md text-xs font-bold ${u.role === 'Admin'
...
//                         <td className="p-4">
//                           <span
//                             className={`px-2.5 py-1 rounded-full text-xs font-bold inline-flex items-center gap-1 ${u.status === 'Active'
//                                 ? 'bg-emerald-50 text-emerald-700 dark:bg-emerald-950/60 dark:text-emerald-300'
//                                 : 'bg-red-50 text-red-700 dark:bg-red-950/60 dark:text-red-300'
//                               }`}
//                           >
...
//                               className={`px-2.5 py-1 rounded-lg text-xs font-semibold transition-colors cursor-pointer ${u.status === 'Active'
...
//                         }}
//                         className={`px-3 py-1 rounded-full text-xs font-bold transition-all cursor-pointer ${settings.paymentGateways[gw.key]
...
//                       }
//                     }}
//                     className={`px-4 py-2 rounded-xl text-xs font-bold cursor-pointer transition-all ${settings.maintenanceMode
...
export const AdminPanel = () => {
  ...
  // ============================================================
  // Admin logout - direct localStorage approach
  // ============================================================
  const handleAdminLogout = () => {
  ...
  // ============================================================
  // Users state - REAL DATABASE USERS
  // ============================================================
  ...
  const getAllUsers = async () => {
    ...
    try {
      ...
      if (response.data.success) {
        setUsersList(response.data.userData || []);
      } else {
        setUsersError(
  ...
  // Fetch users when AdminPanel loads
  useEffect(() => {
    getAllUsers();
  }, []);
  ...
  // Orders / Settings
  // ============================================================
  const [ordersList] = useState(INITIAL_ADMIN_ORDERS);
  const [settings, setSettings] = useState(INITIAL_ADMIN_SETTINGS);
  ...
  const handleAddProduct = (e) => {
    ...
    const createdItem = {
      ...
      description:
        ...
        'Premium curated catalog product.',
      images: [
        newProduct.image ||
          'https://images.unsplash.com/photo-1523275335684-37898b6baf30?w=800&auto=format&fit=crop&q=60',
      ],
  ...
  return (
    <div
      id="admin_panel_page"
      className="min-h-screen bg-gray-100 dark:bg-gray-950 text-gray-900 dark:text-gray-100 flex flex-col font-sans"
    >
      {/* ==========================================================
          HEADER
      ========================================================== */}
      <header className="sticky top-0 z-40 bg-slate-900 text-white border-b border-slate-800 shadow-lg">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          <div className="flex items-center gap-4">
            <Link
              to="/"
              className="flex items-center gap-2 px-3.5 py-1.5 rounded-lg bg-indigo-600 hover:bg-indigo-500 text-white font-bold text-xs sm:text-sm transition-all shadow-md"
            >
              <ArrowLeft className="h-4 w-4" />
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="font-black text-lg tracking-tight">
                ECOBAZAR{' '}
                <span className="text-indigo-400 font-normal text-xs uppercase tracking-widest ml-1">
                  Admin Panel
                </span>
              </span>
            </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              onClick={handleAdminLogout}
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-red-600/90 hover:bg-red-600 text-white text-xs font-bold transition-all border border-red-500/50 ml-1"
              title="Log Out"
            >
              <LogOut className="h-4 w-4" />
              <span className="hidden sm:inline">Log Out</span>
        ...
        <div className="bg-slate-950/80 border-t border-slate-800/80">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex overflow-x-auto gap-1 sm:gap-2 py-2">
            <button
              onClick={() => setActiveTab('dashboard')}
              className={`flex items-center gap-2 px-4 py-2 rounded-lg text-xs sm:text-sm font-bold whitespace-nowrap ${
                activeTab === 'dashboard'
            ...
            >
              <ShoppingBag className="h-4 w-4" />
              <span>2. Products</span>
              <span className="ml-1 px-1.5 rounded-full text-[10px] bg-indigo-900/80 text-indigo-200">
                {productsList.length}
              </span>
            </button>
            ...
            >
              <Users className="h-4 w-4" />
              <span>3. Users</span>
              <span className="ml-1 px-1.5 rounded-full text-[10px] bg-slate-800 text-slate-300">
                {usersList.length}
              </span>
            </button>
        ...
        {activeTab === 'dashboard' && (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="bg-white dark:bg-gray-900 p-6 rounded-2xl border border-gray-200 dark:border-gray-800 shadow-sm space-y-4">
                <h3 className="text-base font-bold flex items-center gap-2 border-b border-gray-100 dark:border-gray-800 pb-4">
                  <Activity className="h-5 w-5 text-emerald-500" />
                  Store Health Monitor
                </h3>
                  ...
                  <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                    {ordersList.map((ord) => (
                      <tr key={ord.id}>
                        ...
                        <td className="p-3">
                          <p className="font-bold">{ord.customer}</p>
                          <p className="text-xs text-gray-500">
                            {ord.email}
                          </p>
                        </td>
          ...
          <div className="space-y-6">
            <div className="bg-white dark:bg-gray-900 p-6 rounded-2xl border border-gray-200 dark:border-gray-800 shadow-sm space-y-4">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                ...
                <button
                  onClick={() => setIsAddProductOpen(true)}
                  className="flex items-center justify-center gap-2 px-4 py-2.5 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl font-bold text-xs sm:text-sm"
                >
                  <Plus className="h-4 w-4" />
                  Add New Product
                </button>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-2">
                <div className="sm:col-span-2 relative">
                  ...
                  <input
                    ...
                    value={productSearch}
                    onChange={(e) => setProductSearch(e.target.value)}
                    className="w-full py-2.5 pl-10 pr-4 text-xs sm:text-sm bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl"
                  />
                </div>
                ...
                <select
                  ...
                  className="w-full py-2.5 px-3 text-xs sm:text-sm bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl"
                >
                  <option value="all">All Categories</option>
                  <option value="electronics">Electronics</option>
                  <option value="apparel">Apparel</option>
                    ...
                    {filteredProducts.map((prod) => (
                      <tr key={prod._id}>
                        ...
                        <td className="p-4 text-right">
                          <button
                            onClick={() => setEditingProduct(prod)}
                            className="p-1.5 text-indigo-600 mr-2"
                          >
                            <Edit3 className="h-4 w-4" />
                          </button>
            ...
            {isAddProductOpen && (
              <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
                <div className="bg-white dark:bg-gray-900 w-full max-w-lg rounded-2xl p-6 shadow-2xl space-y-4">
                  ...
                  >
                    <input
                      ...
                      className="w-full p-2.5 border rounded-lg dark:bg-gray-800"
                    />
          ...
          <div className="space-y-6">
            <div className="bg-white dark:bg-gray-900 p-6 rounded-2xl border border-gray-200 dark:border-gray-800 shadow-sm space-y-4">
              <div>
                ...
                <p className="text-xs text-gray-500 mt-1">
                  All registered users loaded directly from the
              ...
              <div className="flex items-center justify-between">
                ...
                <button
                  ...
                  className="px-3 py-2 bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 text-white rounded-lg text-xs font-bold"
                >
                  ...
                  <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                    {usersLoading ? (
                      ...
                      filteredUsers.map((user) => {
                        const isActive = user.isHold === false;
                        ...
                        return (
                          ...
                          >
                            ...
                            <td className="p-4">
                              <div className="flex items-center gap-3">
                                <div className="h-10 w-10 rounded-full bg-indigo-600 flex items-center justify-center text-white font-bold uppercase">
                                  {(user.fullName || 'U')
                                    .charAt(0)}
                                </div>
                            ...
                            {/* Role */}
                            <td className="p-4">
                              <span className="px-2.5 py-1 rounded-md text-xs font-bold bg-gray-100 dark:bg-gray-800 text-gray-800 dark:text-gray-300 capitalize">
                                {user.role || 'user'}
                              </span>
                            </td>
        ...
        {activeTab === 'others' && (
          <div className="space-y-6">
            <div className="bg-white dark:bg-gray-900 p-6 rounded-2xl border border-gray-200 dark:border-gray-800 shadow-sm">
              ...
              <p className="text-xs text-gray-500 mt-1">
                Store configuration and payment settings.
              </p>
            </div>
                    ...
                    <button
                      onClick={() => {
                        ...
                        setSettings({
                          ...
                          paymentGateways: {
                            ...settings.paymentGateways,
                            [gateway.key]: nextState,
                          },
                        });
                      }}
                      className={`px-3 py-1 rounded-full text-xs font-bold ${
                        settings.paymentGateways[gateway.key]
                          ? 'bg-emerald-500 text-white'
              ...
              <div className="bg-white dark:bg-gray-900 p-6 rounded-2xl border border-gray-200 dark:border-gray-800 shadow-sm space-y-4">
                ...
                <p className="text-xs text-gray-500">
                  {settings.maintenanceMode
                    ? 'ACTIVE - Site in upkeep'
                    : 'OFF - Live for customers'}
                </p>
